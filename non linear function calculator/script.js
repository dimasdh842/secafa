...
setInterval(() => {
    document.querySelector(".container-loader").classList.add("opacity-0")
    document.querySelector(".container-loader").classList.add("opacity-0")
    document.querySelector(".container").classList.add("display-block")
},3000);
...
function bisection(a,b,e) {
    ...
    let x = (a+b)/2
    let i = 0
    //perulangan untuk metode biseksi
	while(i<20 && Math.abs(f(a))>e){
		x = (a+b)/2; 
        
        console.log("x : "+ x + "f(x) : " + f(x));
        let div = document.createElement("div")
        div.classList.add("step-result-container")
        div.setAttribute("id",`step-${i}`)
        div.innerHTML = `<div class="step-result">Langkah ke-${i} x = ${x} , f(x) = ${f(x)} </div>`
        container.appendChild(div)
        ...
        let div_2 = document.createElement("div")
        div_2.classList.add("step-result")
        div_2.innerHTML = `<b>X </b> : ${x}`
        let element = document.querySelector(`#step-${i-1}`).appendChild(div_2)
        console.log(element)
        document.getElementById("hasil-ahkir").innerText = x

}

function cariNilaiDi(x,apangkat,c) {
	return Math.log(Math.pow(x,apangkat)+c); 
}
function turunan(x,c) {
	return (2*x)/(Math.pow(x,2)+c);
}


function FixedPointIteration(x,e,n) {
    let i,max_iterasi = n,x0
     
    for(i=0; i<max_iterasi; i++){
		x0 = x; 
        console.log(x0,apangkat,c);
		x = cariNilaiDi(x0,apangkat,c); //fungsi g(x) yang baru 
		
		if(Math.abs(x-x0)<e){
			console.log(`langkah ke - ${i}	x : ${x} f(x) :	${f(x)}	 \n`); 
			console.log("nilai akar = "+ x+"\n");
			console.log("nilai turunan f(x) = "+ turunan(x0,c) +" \n");
			console.log("selesai pada iterasi = " + i+ "\n" ); 
			return 0; 
		}
        console.log(`langkah ke - ${i}	x : ${x} f(x) :	${f(x)}	 \n`); 
	}
	console.log("nilai akar = "+x+" dengan total iterasi = " + i--);
}